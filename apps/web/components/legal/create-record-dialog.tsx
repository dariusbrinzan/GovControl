...
import { apiGet, apiPost, apiUpload, type CourtDecision, type LegalCase, type LegalObligation } from "../../lib/api";
...
export function CreateRecordDialog({ view, token, onCreated }: { view: OperationalView; token: string; onCreated: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (view === "documents") { const file = data.get("file"); if (!(file instanceof File) || !file.size) throw new Error("Selectează un fișier."); await apiUpload("/documents", token, data); }
...
function fields(view: OperationalView, cases: LegalCase[], decisions: CourtDecision[], obligations: LegalObligation[]) {
  ...
  return <><input name="entity_type" type="hidden" value="LegalObligation" /><label className="full">ID obligație<input name="entity_id" required /></label><label>Categorie<input name="category" required value="SUPPORTING_DOCUMENT" readOnly /></label><label>Fișier<input name="file" required type="file" /></label></>;
}
