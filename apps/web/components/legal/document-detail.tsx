...
export function DocumentDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const canInspectDeleted = user?.permissions.includes("documents.delete") ?? false;
      const canAudit = user?.permissions.includes("documents.audit") ?? false;
      const [record, versionItems, auditItems] = await Promise.all([
        documentsGet(id, token, canInspectDeleted),
        documentsVersions(id, token),
        canAudit ? documentsAudit(id, token) : Promise.resolve([]),
      ]);
